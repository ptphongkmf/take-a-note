...
const SheetContent = <T extends ValidComponent = "div">(
  ...
  return (
    <SheetPortal position={local.position}>
      ...
      >
        {local.children}
        <SheetPrimitive.CloseButton class="focus:ring-ring data-[state=open]:bg-secondary absolute top-4 right-4 cursor-pointer rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:pointer-events-none">
          <svg
...
const SheetHeader: Component<ComponentProps<"div">> = (props) => {
  ...
  return (
    <div
      class={c("flex flex-col space-y-2 text-center sm:text-left", local.class)}
      {...others}
    />
  );
};
...
const SheetTitle = <T extends ValidComponent = "h2">(
  ...
  return (
    <SheetPrimitive.Title
      class={c("text-foreground text-lg font-semibold", local.class)}
      {...others}
    />
  );
};
...
const SheetDescription = <T extends ValidComponent = "p">(
  ...
  return (
    <SheetPrimitive.Description
      class={c("text-muted-foreground text-sm", local.class)}
      {...others}
    />
  );
};
