...
import {
  noteQueryListKey,
} from "#entities/note/api/queries/note-list-query.ts";

export const Route = createFileRoute("/notes/$id")({
  loader: async ({ params }) => {
    queryClient.prefetchQuery({ queryKey: noteQueryListKey.all() });

    await queryClient.fetchQuery({
      queryKey: noteQueryDetailOrDefaultKey.detailOrDefault(params.id),
    }).catch(/* ignore */);
